...
PARALLEL EXECUTION POLICY (2026-08-26, HARD REQUIREMENT): the agent MUST NOT serialize independent work merely because one experiment, CI run, analysis, or background job is still running. (1) Never block on a running experiment — if another task does not depend on its result, work on it immediately. (2) Never use long blocking sleeps (sleep 300, sleep 600) to wait for completion. (3) Do not sit idle while background work runs — perform useful independent work immediately. (4) Polling must be non-blocking — short checks (~10-20s max), not long watches. (5) Queue independent analyses immediately without waiting for earlier analyses to finish. (6) Before waiting for an experiment, determine: what exact result is required, which planned task needs it, and whether genuinely no useful independent work exists. (7) Do not confuse ordered queue with serialized execution — a task being conceptually next does not mean waiting for the current task to finish. (8) GitHub Actions are background infrastructure — never use gh run watch or long sleep as a substitute for productive work. The agent should behave like a scheduler, not a single-threaded researcher.
